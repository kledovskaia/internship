...
function handleInputChange() {
    ...
    if (this.value.split('.').length > 2) this.value = this.value.split('.').slice(0, 2).join('.');
}
...
async function stacking(event) {
    ...
    while(state.stacking[id].daysLeft){
        ...
        await delay();
        state.stacking[id].profit += Math.log10(deposit + profit) * config.coefficient[period] / 100;
...
async function renderTable() {
    ...
    ${Object.values(state.stacking).map((entity)=>`<tr>
